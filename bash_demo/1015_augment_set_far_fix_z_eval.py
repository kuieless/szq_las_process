...
def _get_train_opts() -> Namespace:
    ...
    parser.add_argument('--dataset_path', type=str, default='/data/yuqi/Datasets/DJI/xiayuan_20240118_demo',required=False, help='')
...
def hello(hparams: Namespace) -> None:
    ...
    used_files = []
    for ext in ('*.png', '*.jpg'):
        used_files.extend(glob(os.path.join(hparams.dataset_path,'train/rgbs', ext)))
    used_files.sort()
    ...
    for metadata_item in tqdm(train_items):
        ...
        # metadata_path = os.path.join(hparams.dataset_path, 'val', 'metadata', f"{metadata_item.image_path.stem}.pt")
        ...
        if not os.path.exists(os.path.join('Output_subset', f'render_far{z_vals_inbound}_val', 'metadata')):
            Path(os.path.join('Output_subset', f'render_far{z_vals_inbound}_val')).mkdir(parents=True, exist_ok=True)
            Path(os.path.join('Output_subset', f'render_far{z_vals_inbound}_val', 'metadata')).mkdir(parents=True, exist_ok=True)

        torch.save(metadata_old, os.path.join('Output_subset', f'render_far{z_vals_inbound}_val', 'metadata', f"{metadata_item.image_path.stem}.pt"))
