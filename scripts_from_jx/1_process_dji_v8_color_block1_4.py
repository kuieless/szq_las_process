...
def _get_opts():
    ...
    parser.add_argument('--original_images_path', default='/data/yuqi/Datasets/DJI/origin/Longhua_origin/block1/origin_image',type=str, required=False)
    parser.add_argument('--original_images_list_json_path', default='/data/yuqi/Datasets/DJI/origin/Longhua_origin/block1/original_image_list.json',type=str, required=False)
    parser.add_argument('--infos_path', default='/data/yuqi/Datasets/DJI/origin/Longhua_origin/block1/BlocksExchangeUndistortAT.xml',type=str, required=False)
    parser.add_argument('--output_path', default='/data/yuqi/code/GP-NeRF-semantic/longhua_block1_test',type=str, required=False)
    ...
    with (output_path / 'mappings.txt').open('w') as f:
        for i, rgb_name in enumerate(tqdm(images_name_sorted)):
            ...
            if True:
                ...
                distortion1 = np.array([float(-0.048050073),
                           float(0.019863319),
                           float(0.000820135),
                           float(-0.000446662),
                           float(-0.093597725)])  # k1 k2 p1 p2 k3


                img1 = cv2.imread(str(original_images_path) + '/' + original_image_name_sorted[i]) 
                camera_matrix1 = np.array([[8199.818, 0, 4089.162],
                                        [0, 8199.818, 2750.072],
                                        [0, 0, 1]])
