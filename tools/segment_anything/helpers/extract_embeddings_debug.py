...
from tools.segment_anything import sam_model_registry, SamPredictor, SamAutomaticMaskGenerator
...
def main(checkpoint_path, model_type, device, images_folder, embeddings_folder):
    ...
    for image_name in tqdm(os.listdir(images_folder)):
        ...
        masks, iou_preds, _ = predictor.predict(np.array([[510, 414]]), in_labels, return_torch=True)

        # masks = mask_generator.generate(image, image_embedding)

        # sam_feature = image_embedding
        # predictor.set_feature(sam_feature, [H, W])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rgbs_path", type=str, default="/data/yuqi/code/GP-NeRF-semantic/logs_dji/1003_yingrenshi_density_depth_hash22_semantic/18/eval_200000_far0.5/pred_rgb")
    ...
    embeddings_folder = os.path.join("/data/yuqi/code/GP-NeRF-semantic/zyq/test")
