...
def get_mask(dict, semantics):
    ...
    mask = torch.zeros_like((dict[0]['segmentation']))
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python combine_sam_m2f.py <path to SAM .npy masks> <path to m2f pngs> <path to save SAM_m2f masks> <path to save SAM_m2f visualized>")
        sys.exit(1)
    SAM_path = sys.argv[1]
    m2f_path = sys.argv[2]
    save_path = sys.argv[3]
    save_path_vis = sys.argv[4]
    ...
    for i in tqdm(range(len(sams))):
        ...
        dict = [{'segmentation': load_dict[k], 'id': get_id(load_dict[k], semantics)} for k in range(len(load_dict))]
        ...
        Image.fromarray(_mask.astype(np.uint16)).save(save_path+sams[i].split('/')[-1][:6]+".png")

        mask_rgb = custom2rgb(_mask)
        Image.fromarray(mask_rgb).save(save_path_vis+sams[i].split('/')[-1][:6]+".png")
        # end = time.time()
        # print(end-start)
