...
def main(hparams):
    ...
    with open('dji/project_script/ply/points_lidar_list_M1.pkl', "rb") as file:
    ...
    for i, rgb_name in enumerate(tqdm(images_name_sorted)):
        ...
        cv2.imwrite('dji/project_script/ply/output/'+'{0:06d}_1_rgbs.png'.format(i), img_change)
        ...
        points_color = points_lidar_list[i][:,3:] 
        if i - 1 >= 0:
            points_color = np.concatenate((points_color, points_lidar_list[i-1][:,3:]), axis=0)
        if i + 1 <= len(images_name_sorted):
            points_color = np.concatenate((points_color, points_lidar_list[i+1][:,3:]), axis=0)
        ...
        for x, y, depth, color in tqdm(zip(projected_points[0], projected_points[1], depth_z, points_color_mask)):
            ...
            if depth < depth_map[y, x]:
                ...
                if step != 0: 
                    ...
                    cv2.circle(image, (x, y), step, (float(color[2]), float(color[1]), float(color[0])), -1)
                else:
                    depth_map[y, x] = depth
                    image[y, x] = color[::-1]
        ...
        cv2.imwrite('dji/project_script/ply/output/'+'{0:06d}_2_project.png'.format(i), image)
