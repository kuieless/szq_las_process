...
def main(hparams):
    ...
    for i, rgb_name in enumerate(tqdm(images_name_sorted)):
        ...
        depth_z = pt_3d_trans_z.reshape(-1, 1)
        x = projected_points[0].astype(int)
        y = projected_points[1].astype(int)

        valid_depth_mask = depth_z < depth_map[y,x]
        valid_indices = np.where(valid_depth_mask)[0]
        
        step = 2
        y_start = np.maximum(0, y[valid_indices] - step)
        y_end = np.minimum(image_height, y[valid_indices] + step)
        x_start = np.maximum(0, x[valid_indices] - step)
        x_end = np.minimum(image_width, x[valid_indices] + step)

        for j in range(len(valid_indices)):
            depth_map[y_start[j]:y_end[j], x_start[j]:x_end[j]] = depth_z[valid_indices[j]]
