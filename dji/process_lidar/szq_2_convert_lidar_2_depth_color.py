...
# ==============================================================================
# (!!!) (新增) 数据集类 (用于多核 CPU 预加载)
# ==============================================================================
class LidarProjectionDataset(Dataset):
    def __init__(self, hparams, images_name_sorted, original_image_name_sorted, metadata_dir, 
                 segment_dir, num_segments, camera_matrix1, distortion_coeffs1, camera_matrix_ori):
        
        self.hparams = hparams
        self.images_name_sorted = images_name_sorted
        self.original_image_name_sorted = original_image_name_sorted
        ...
        self.distortion_coeffs1 = distortion_coeffs1
        self.camera_matrix_ori = camera_matrix_ori
        self.original_images_path = Path(hparams.original_images_path)
        self.down_scale = hparams.down_scale
        self.image_width = int(5472 / hparams.down_scale)
        self.image_height = int(3648 / hparams.down_scale)

        # 加载坐标系 (CPU)
        coordinate_info = torch.load(Path(hparams.dataset_path) / 'coordinates.pt')
        self.origin_drb = coordinate_info['origin_drb'].to(dtype=torch.float32)
        self.pose_scale_factor = coordinate_info['pose_scale_factor']

        # 变换矩阵 (CPU)
        self.ZYQ_cpu = torch.tensor([[0, 0, -1], [0, 1, 0], [1, 0, 0]], dtype=torch.float64)
        r135 = rad(135)
        self.ZYQ_1_cpu = torch.tensor(
            [[1, 0, 0], [0, math.cos(r135), math.sin(r135)], [0, -math.sin(r135), math.cos(r135)]], dtype=torch.float64
        )

    def __len__(self):
        # 我们只处理到 hparams.end
        return min(self.num_segments, len(self.images_name_sorted), self.hparams.end)

    def __getitem__(self, i):
        # 这个函数将在一个单独的 CPU 进程中被调用
        # (过滤开始/结束帧)
        if i < self.hparams.start:
            # 对于 DataLoader，返回 None 或空字典是处理“跳过”的标准方式
            return None 

        # --- 1. 加载图像并校正 (CPU) ---
        img1 = cv2.imread(str(self.original_images_path) + '/' + self.original_image_name_sorted[i])
        if img1 is None:
            tqdm.write(f"警告: 无法读取图像 {self.original_image_name_sorted[i]}，跳过帧 {i}")
            return None
        
        img_change = cv2.undistort(img1, self.camera_matrix1, self.distortion_coeffs1, None, self.camera_matrix_ori)
        img_change = cv2.resize(img_change, (self.image_width, self.image_height))
        
        # --- 2. 加载元数据 (CPU) ---
        metadata_path = self.metadata_dir / f'{i:06d}.pt'
        if not metadata_path.exists():
            tqdm.write(f"警告: 元数据文件 {metadata_path} 未找到，跳过帧 {i}")
            return None
        metadata = torch.load(metadata_path, map_location='cpu')
        ...
        try:
            segment_path_current = self.segment_dir / f'{i:06d}.npy'
            points_data_current = np.load(segment_path_current)
            points_nerf_list.append(points_data_current[:, :3])
            if self.hparams.debug: points_color_list.append(points_data_current[:, 3:])
        except FileNotFoundError:
            tqdm.write(f"警告: 找不到核心文件 {segment_path_current}。跳过帧 {i}")
            return None # 核心帧丢失，必须跳过

        if i - 1 >= 0:
            try:
                segment_path_prev = self.segment_dir / f'{i - 1:06d}.npy'
                points_data_prev = np.load(segment_path_prev)
                points_nerf_list.append(points_data_prev[:, :3])
                if self.hparams.debug: points_color_list.append(points_data_prev[:, 3:])
            except FileNotFoundError: pass

        if i + 1 < self.num_segments:
            try:
                segment_path_next = self.segment_dir / f'{i + 1:06d}.npy'
                points_data_next = np.load(segment_path_next)
                points_nerf_list.append(points_data_next[:, :3])
                if self.hparams.debug: points_color_list.append(points_data_next[:, 3:])
            except FileNotFoundError: pass

        points_nerf_cpu_np = np.concatenate(points_nerf_list, axis=0)
        
        if points_nerf_cpu_np.shape[0] == 0:
            return {'frame_idx': i, 'is_empty': True} # 返回空帧

        points_color_cpu_np = None
        if self.hparams.debug:
            points_color_cpu_np = np.concatenate(points_color_list, axis=0)

        # --- 4. 坐标系变换 (CPU) ---
        # (在 CPU worker 中使用 PyTorch)
        points_nerf_cpu_t = torch.tensor(points_nerf_cpu_np, dtype=torch.float64)
        points_nerf_cpu_t = self.ZYQ_cpu @ points_nerf_cpu_t.T
        points_nerf_cpu_t = (self.ZYQ_1_cpu @ points_nerf_cpu_t).T
        points_nerf_cpu_t = points_nerf_cpu_t.to(dtype=torch.float32)
        points_nerf_cpu_t = (points_nerf_cpu_t - self.origin_drb) / self.pose_scale_factor

        # --- 5. 计算 W2C (CPU) ---
        c2w_3x4_cpu = metadata['c2w'].to(dtype=torch.float32) # (3, 4)
        E2 = c2w_3x4_cpu.numpy()
        E2_transformed = np.stack([
            E2[:, 0], E2[:, 1] * -1, E2[:, 2] * -1, E2[:, 3]
        ], axis=1)
        E2_4x4 = np.concatenate((E2_transformed, [[0, 0, 0, 1]]), axis=0)
        
        try:
            w2c_np = np.linalg.inv(E2_4x4)
        except np.linalg.LinAlgError:
            tqdm.write(f"警告: 帧 {i} 的 c2w 矩阵求逆失败。跳过此帧。")
            return None

        # --- 6. 内参 (CPU) ---
        camera_matrix_np = np.array([[metadata['intrinsics'][0] / self.down_scale, 0, metadata['intrinsics'][2] / self.down_scale],
                                     [0, metadata['intrinsics'][1] / self.down_scale, metadata['intrinsics'][3] / self.down_scale],
                                     [0, 0, 1]], dtype=np.float32)
        ...
        result = {
            'frame_idx': i,
            'is_empty': False,
            'points_nerf': points_nerf_cpu_t, # (N, 3) float32 CPU Tensor
            'w2c': torch.tensor(w2c_np, dtype=torch.float32), # (4, 4) float32 CPU Tensor
            'intrinsics': torch.tensor(camera_matrix_np, dtype=torch.float32), # (3, 3) float32 CPU Tensor
            'image_height': self.image_height,
            'image_width': self.image_width,
            'img_change': img_change if self.hparams.debug else None,
            'points_color': points_color_cpu_np if self.hparams.debug else None
        }
        
        return result

def custom_collate_fn(batch):
    """过滤掉 __getitem__ 返回 None 的项"""
    batch = [item for item in batch if item is not None]
    if not batch:
        return None
    # 默认的 collate 行为 (因为 batch_size=1, batch 列表里只有一个元素)
    return batch[0]
...
def main(hparams):
    ...
    log_file_path = dataset_path / 'projection_log.txt'
    log_file = open(log_file_path, 'w')
    log_file.write(f"--- 点云投影日志 (GPU + CPU 多核版) ---\n")
    log_file.write(f"数据集路径: {dataset_path}\n")
    log_file.write(f"CPU Workers (-j): {hparams.num_workers}\n")
    log_file.write(f"处理帧范围: {hparams.start} 到 {hparams.end}\n")
    log_file.write("-" * 30 + "\n")
    ...
    if torch.cuda.is_available():
        ...
        device = torch.device("cpu")
        print("--- 未检测到 CUDA。将使用 CPU。 ---")
    print(f"--- 将使用 {hparams.num_workers} 个 CPU 核心进行数据预加载。 ---")
    ...
    # (!!!) (修复) 确保 JSON 和 XML 的匹配是正确的
    # (假设 XML 和 JSON 列表是一一对应的)
    min_len = min(len(sorted_json_data), len(images_name))
    if min_len != len(images_name) or min_len != len(sorted_json_data):
        print(f"警告: JSON ({len(sorted_json_data)}) 和 XML ({len(images_name)}) 数量不匹配! "
              f"将只处理 {min_len} 条记录。")

    for i in tqdm(range(min_len), desc="匹配 XML 与 JSON"):
        sorted_process_line = {}
        json_line = sorted_json_data[i] # (!!!) (修复) 使用 sorted_json_data
        sorted_process_line['images_name'] = images_name[i]
        sorted_process_line['pose'] = xml_pose[i, :]
        path_segments = json_line['origin_path'].split('/')
        last_two_path = '/'.join(path_segments[-2:])
    ...
    # ==============================================================================
    # --- 4. (!!!) (多核) 创建 Dataset 和 DataLoader ---
    # ==============================================================================
    print("\n" + "=" * 20 + " 步骤 4: 创建 Dataset 和 DataLoader " + "=" * 20)
    
    dataset = LidarProjectionDataset(
        hparams=hparams,
        images_name_sorted=images_name_sorted,
        original_image_name_sorted=original_image_name_sorted,
        metadata_dir=metadata_dir,
        segment_dir=segment_dir,
        num_segments=num_segments,
        camera_matrix1=camera_matrix1,
        distortion_coeffs1=distortion_coeffs1,
        camera_matrix_ori=camera_matrix_ori
    )
    ...
    loader = DataLoader(
        ...
        num_workers=hparams.num_workers, # (!!!) 并行核心
        collate_fn=custom_collate_fn,   # (!!!) 用于跳过 None
        pin_memory=True # (!!!) 加速 CPU -> GPU 传输
    )

    # ==============================================================================
    # --- 5. (!!!) (GPU) 循环处理 DataLoader ---
    # ==============================================================================
    print("\n" + "=" * 20 + " 步骤 5: 开始投影 (GPU + CPU 多核) " + "=" * 20)

    # (!!!) (核心) 主循环现在遍历 DataLoader
    for batch in tqdm(loader, desc="投影点云 (并行加载)"):
        ...
        # --- A. 将预加载的数据移动到 GPU ---
        points_nerf_gpu = batch['points_nerf'].to(device)
        w2c_gpu = batch['w2c'].to(device)
        camera_matrix_gpu = batch['intrinsics'].to(device)
        total_points_matched = points_nerf_gpu.shape[0]
        ...
        # --- C. (GPU) 过滤点 (与之前相同) ---
        image_width, image_height = batch['image_width'], batch['image_height']
        mask_x_gpu = (projected_points_gpu[0, :] >= 0) & (projected_points_gpu[0, :] < image_width)
        mask_y_gpu = (projected_points_gpu[1, :] >= 0) & (projected_points_gpu[1, :] < image_height)
        mask_z_gpu = pt_3d_trans_gpu[2, :] > 0
        mask_gpu = mask_x_gpu & mask_y_gpu & mask_z_gpu
        ...
        if hparams.debug == False:
            ...
            # (Debug 可视化逻辑)
            img_change = batch['img_change']
            points_color = batch['points_color']
            ...
            sorted_colors = points_color_mask[indices]
            final_colors = sorted_colors[mask_unique]
            image[final_iy, final_ix] = final_colors[:, ::-1] # BGR
            Image.fromarray(image).save(str(output_path / 'debug' / f'{i:06d}_2_project.png'))
...
if __name__ == '__main__':
    # (确保已在 _get_opts 中添加了 --num_workers)
    main(_get_opts())