# '''
# 后来佳兴修改过的， from https://github.com/zyqz97/GPNeRF-semantic-dataProcess/blob/main/2_render_mesh_depth.py
# '''
...
#     tmp = root.findall('Block/Photogroups/Photogroup/Photo/Pose')
#     translation = np.array(root.find('SRSOrigin').text.split(','), dtype=float)#.astype(np.float)
...
def load_mesh(args, obj_filename, nerf_metadata=None, save_mesh=False):
    # ...
    verts, faces, aux = load_obj(obj_filename, device=device,
                                 load_textures=False, # <--- 修改这里
                                 create_texture_atlas=False, # 建议也关掉
                                 # texture_atlas_size=8,   # 可以注释掉
                                 # texture_wrap=None,      # 可以注释掉
                                 )
    # ...
...
def load_nerf_metadata(args, metadata_dir, filename, verbose=True):
    ...
    if srs_origin_element is not None and srs_origin_element.text:
        # 如果找到了SRSOrigin标签，就正常读取
        translation = np.array(srs_origin_element.text.split(','), dtype=float)
        print("Found <SRSOrigin> in XML, using it as translation.")
    else:
        # 如果没找到，就打印一个警告，并设置translation为0
...
def convert_verts_to_nerf_space(verts, nerf_metadata):
    ...
    # pose_scale_factor = torch.tensor(nerf_metadata['coordinates']['pose_scale_factor'].item()).to(torch.float64)
    # AFTER (corrected)
    pose_scale_factor = torch.tensor(nerf_metadata['coordinates']['pose_scale_factor']).to(torch.float64)
    translation = torch.from_numpy(nerf_metadata['translation']).float().to(device)
...
def main(args):
    ...
    for name in tqdm(names[:]):
        ...
        if args.simplify == 1:
            # np.save(os.path.join(save_dir+ '/' + args.split + '/depth_mesh', '%s.npy' % filename), depth[:,:,None].astype(np.float16))
            np.save(os.path.join(save_dir+ '/' + args.split + '/depth_mesh', '%s.npy' % filename), depth[:,:,None].astype(np.float32))
        ...
        if args.visualize:
            ...
            rgb_np_color = cv2.cvtColor(rgb_np_uint8, cv2.COLOR_GRAY2BGR) # Convert grayscale to BGR
...
'''
python 3_render_mesh_depthcc.py --obj_filename /home/data1/szq/Megadepth/Aerial_lifting_early/dji/process_lidar/Mesh.obj --nerf_metadata_dir /home/data1/szq/Megadepth/Aerial_lifting_early/dji/process_lidar/output7 --metadataXml_path /home/data1/szq/Megadepth/Aerial_lifting_early/dji/process_lidar/BlocksExchangeUndistortAT.xml --save_dir /home/data1/szq/Megadepth/Aerial_lifting_early/dji/process_lidar/output7 --split "val" --down 1.0
