...
#     sorted_process_data = []
#
#     # --- ↓↓↓ 用这个修正后的循环，替换掉你脚本里原来的 for 循环 ↓↓↓ ---
#     for i in tqdm(range(len(sorted_json_data)), desc="load meta_data"):
#         sorted_process_line = {}
#         json_line = json_data[i]
#
#         # 修正：不再按文件名匹配，直接假设顺序一致并使用索引 i
...
def main(hparams):
    ...
    # Use corrected loop assuming the order in xml and json is consistent
    for i in tqdm(range(len(sorted_json_data)), desc="load meta_data"):
        sorted_process_line = {}
        json_line = json_data[i]

        # Assuming direct correspondence by index
        sorted_process_line['images_name'] = images_name[i]
        sorted_process_line['pose'] = xml_pose[i, :]
    ...
    if hparams.debug:
        ...
        (dataset_path / 'val' / 'depth_dji').mkdir(parents=True, exist_ok=True)
        (dataset_path / 'train' / 'depth_dji').mkdir(parents=True, exist_ok=True)
    ...
    for i, rgb_name in enumerate(tqdm(images_name_sorted)):
        ...
        if i % int(len(images_name_sorted) / hparams.num_val) == 0:
            split_dir = dataset_path / 'val'
        else:
            split_dir = dataset_path / 'train'
        ...
        # Load camera pose for the current frame
        metadata = torch.load(str(split_dir) + f'/metadata/{i:06d}.pt', map_location='cpu')
        camera_rotation = metadata['c2w'][:3, :3]
        camera_position = metadata['c2w'][:3, 3]
        camera_matrix = np.array([[metadata['intrinsics'][0] / down_scale, 0, metadata['intrinsics'][2] / down_scale],
                                  [0, metadata['intrinsics'][1] / down_scale, metadata['intrinsics'][3] / down_scale],
        ...
        if hparams.debug == False:
            np.save(split_dir / 'depth_dji' / f'{i:06d}.npy', depth_map)
        else:
...
'''
SBMU python 2_convert_lidar_2_depth_color.py --dataset_path F:\download\SMBU2\Aerial_lifting_early\dji\process_lidar\output --original_images_path F:\download\SMBU2\smbu11161103\images\survey --infos_path F:\download\SMBU2\smbu11161103\AT\BlocksExchangeUndistortAT.xml --original_images_list_json_path F:\download\SMBU2\smbu11161103\images\survey\image_list.json --las_output_path F:\download\SMBU2\Aerial_lifting_early\dji\process_lidar\output --num_val 10 --start 0 --end 107 --down_scale 4 --fx 3691.094 --fy 3691.094 --cx 2755.620 --cy 1797.584 --k1 0.002214303 --k2 -0.009188852 --p1 -0.002608224 --p2 0.001401760 --k3 0.007309819


SBMU python 2_convert_lidar_2_depth_color.py --dataset_path F:\download\SMBU2\Aerial_lifting_early\dji\process_lidar\output --original_images_path H:\BaiduNetdiskDownload\LiDAR_Raw\SZIIT\PSZIIT018\images\survey --infos_path H:\BaiduNetdiskDownload\LiDAR_Raw\SZIIT\PSZIIT018\AT\BlocksExchangeUndistortAT.xml --original_images_list_json_path H:\BaiduNetdiskDownload\LiDAR_Raw\SZIIT\PSZIIT018\images\survey\image_list.json --las_output_path F:\download\SMBU2\Aerial_lifting_early\dji\process_lidar\output --num_val 10 --start 0 --end 107 --down_scale 4 --fx 3692.240 --fy 3691.094 --cx 2755.094 --cy 1796.394 --k1 0.001630065 --k2 -0.007661543 --p1 -0.002702158 --p2 0.001345463 --k3 0.006244946
