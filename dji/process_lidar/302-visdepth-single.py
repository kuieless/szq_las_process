...
# # python 302-visdepth-single.py /home/data1/szq/Megadepth/Aerial_lifting_early/dji/process_lidar_mesh_szq/GES/output/train/depth_mesh
...
#         if valid_depths.size == 0:
#             print(f"⚠️ 警告: {input_file_path.name} 中未找到任何有效的深度值（在 0m 和 {max_dist or 'inf'}m 之间）。")
#             min_depth, max_depth = 0.0, 1.0 # 使用默认范围
#         else:
#             # [!! 修改 !!] 使用传入的百分位数
...
#         # [!! 修改 !!] 更新打印信息
#         print(f"📊 自动检测到的深度范围 ({min_perc}%-{max_perc}%): {min_depth:.2f}m - {max_depth:.2f}m")
...
#             depth_vis = (depth_vis - min_depth) / (max_depth - min_depth)
#         else:
#             depth_vis = np.zeros_like(depth_vis) # 如果 min==max，则全为0
...
#         # [!! 修改 !!] 将所有无效区域（0值 或 超过max_dist的值）设为黑色
#         # valid_mask 已经被正确设置
#         colorized_depth[~valid_mask] = [0, 0, 0]
...
#         print(f"❌ 错误: 百分位数设置无效。最小值 {min_perc} 必须小于最大值 {max_perc}。")
#         sys.exit(1)
        
#     # [!! 新增 !!] 获取最大距离
#     max_dist = args.max_dist
#     if max_dist is not None and max_dist <= 0:
#         print(f"❌ 错误: --max_dist 必须是一个正数。")
#         sys.exit(1)
...
#         print(f"--- 模式: 单文件处理 ---")
#         # [!! 修改 !!] 传递新参数
#         process_depth_file(input_path, final_output_file, min_perc, max_perc, max_dist)
...
#             final_output_file = output_dir / f"{npy_file.stem}_visualization.png"
#             # [!! 修改 !!] 传递新参数
#             if process_depth_file(npy_file, final_output_file, min_perc, max_perc, max_dist):
#                 success_count += 1
#             else:
#                 fail_count += 1
...
#     # [!! 新增 !!] 添加新参数
#     parser.add_argument("--percentiles", nargs=2, type=float, default=[2.0, 98.0],
#                         help="(可选) 用于归一化的最小和最大百分位数。默认: 2.0 98.0")
#     parser.add_argument("--max_dist", type=float, default=None,
#                         help="(可选) 视为有效的最大深度值（米）。超过此值的将显示为黑色。默认: 无限制")
...
from pathlib import Path
import sys
from typing import Union # <--- 修复：第1步，添加此导入
...
def process_depth_file(
    ...
    min_perc: float, 
    max_perc: float, 
    max_dist: Union[float, None] # <--- 修复：第2步，将 'float | None' 改为 'Union[float, None]'
):
    """
    加载、处理并可视化单个NPY深度图文件。
    """
...
python 302-visdepth-single.py /path/to/my/000010_cleaned.npy --max_dist 1000
