# 脚本功能：
# 1. (新增) 加载归一化的 .npy 深度图，将其乘以缩放因子，恢复为真实的米制单位。
# 2. (新增) 将恢复了真实尺度的 .npy 深度图保存到新的 'depth_metric' 文件夹中。
# 3. 同时，保留原有的可视化功能，生成包含RGB图、彩色深度图和真实尺度标尺的对比图。
...
def main(hparams):
    ...
    (metric_output_path / 'train' / 'depth_dji').mkdir(parents=True, exist_ok=True)
    (metric_output_path / 'val' / 'depth_dji').mkdir(parents=True, exist_ok=True)
    ...
    for npy_path in tqdm(all_depth_paths, desc="正在处理深度图"):
        ...
        # 保存恢复尺度后的 .npy 文件
        metric_npy_output_path = metric_output_path / split_name / 'depth_dji' / npy_path.name
...
if __name__ == '__main__':
    main(_get_opts())
#   python 302-visdepth.py --dataset_path /home/data1/szq/Megadepth/Aerial_lifting_early/dji/process_lidar_mesh_szq/output/hav055
