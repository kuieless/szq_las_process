...
def main(hparams):
    ...
    (output_path / 'train' / 'AT').mkdir(parents=True,exist_ok=True)
    (output_path / 'train' / 'ori').mkdir(parents=True,exist_ok=True)
    (output_path / 'train' / 'change').mkdir(parents=True,exist_ok=True)
    (output_path / 'train' / 'error_13').mkdir(parents=True,exist_ok=True)
    (output_path / 'train' / 'error_18').mkdir(parents=True,exist_ok=True)
    ...
    images_name_sorted=images_name_sorted[::100]
    original_image_name_sorted=original_image_name_sorted[::100]
    with (output_path / 'mappings.txt').open('w') as f:
        for i, rgb_name in enumerate(tqdm(images_name_sorted)):
            ...
            if True:
                ...
                cv2.imwrite(str(split_dir / 'AT' / '{0:06d}.jpg'.format(i)), distorted)


                img1 = cv2.imread(str(original_images_path) + '/' + original_image_name_sorted[i]) 
                cv2.imwrite(str(split_dir / 'ori' / '{0:06d}.jpg'.format(i)), img1)
                ...
                a = cv2.undistort(img1, camera_matrix1, distortion1, None, camera_matrix)
                cv2.imwrite(str(split_dir / 'change' / '{0:06d}.jpg'.format(i)), a)


                difference = mse(distorted, img1)
                error_image = create_error_image(distorted, img1)
                color_map_image = convert_to_color_map(error_image)
                cv2.imwrite(str(split_dir / 'error_13' / '{0:06d}.jpg'.format(i)), color_map_image)

                difference = mse(distorted, a)
                error_image = create_error_image(distorted, a)
                color_map_image = convert_to_color_map(error_image)
                cv2.imwrite(str(split_dir / 'error_18' / '{0:06d}.jpg'.format(i)), color_map_image)
                
                # w,h=5472, 3648
                # newcameramtx, roi = cv2.getOptimalNewCameraMatrix(camera_matrix1, distortion1, (w,h), 1, (w,h))
