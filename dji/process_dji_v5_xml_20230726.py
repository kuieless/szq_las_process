...
def _get_opts():
    ...
    parser.add_argument('--images_path', default='/media/yuqi/60D42BF3D42BCA5C/Users/yuqi/Documents/DJI/DJITerra/18965401139/1202_AT/AT/.temp/ImageCorrection/undistort',type=str, required=False)
    parser.add_argument('--infos_path', default='/media/yuqi/60D42BF3D42BCA5C/Users/yuqi/Documents/DJI/DJITerra/18965401139/1202_AT/AT/BlocksExchangeUndistortAT.xml',type=str, required=False)
    parser.add_argument('--output_path', default='/disk1/Datasets/dji/dji-XML',type=str, required=False)
...
def main(hparams):
    ...
    for i in range(len(c2w_R)):
        ...
        # temp = torch.hstack((ZYQ @ temp[:3, :3] @ torch.inverse(ZYQ), ZYQ @ temp[:3, 3:]))

        temp = torch.hstack((ZYQ_1 @ temp[:3, :3], ZYQ_1 @ temp[:3, 3:]))
        # temp = torch.hstack((ZYQ_1 @ temp[:3, :3] @ torch.inverse(ZYQ_1), ZYQ_1 @ temp[:3, 3:]))
    ...
    with (output_path / 'mappings.txt').open('w') as f:
        for i, rgb_name in enumerate(tqdm(images_name)):
            ...
            # label = 'others'
            # omega, phi, kappa = xml_pose[i][3], xml_pose[i][4], xml_pose[i][5]
