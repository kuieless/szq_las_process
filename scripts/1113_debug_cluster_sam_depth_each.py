...
def _get_train_opts() -> Namespace:
    ...
    parser.add_argument('--panoptic_dir', type=str, default='/data/yuqi/code/GP-NeRF-semantic/logs_longhua_b2/1108_longhua_b2_density_depth_hash22_instance_origin_sam_0.001_depth_crossview_all/2/eval_100000/panoptic',required=False, help='')
...
def hello(hparams) -> None:
    ...
    for cluster_size in cluster_sizes:
        ...
        # 先用采样点进行聚类，得到聚类中心后， 对所有的 things feature进行聚类
        _, centroids = cluster(all_thing_features_mean.cpu().numpy(), bandwidth=bandwidth, device=device, num_images=eval_num+train_num, 
                                    num_points=num_points, use_silverman=use_silverman, use_dbscan=use_dbscan,cluster_size=cluster_size, all_centroids=None, use_mean=True)
