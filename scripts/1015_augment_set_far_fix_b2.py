...
def hello(hparams: Namespace) -> None:
    ...
    for metadata_item in tqdm(train_items):
        ...
        if not os.path.exists(os.path.join('Output_subset_b2', f'render_far{z_vals_inbound}', 'metadata')):
            Path(os.path.join('Output_subset_b2', f'render_far{z_vals_inbound}')).mkdir(parents=True, exist_ok=True)
            Path(os.path.join('Output_subset_b2', f'render_far{z_vals_inbound}', 'metadata')).mkdir(parents=True, exist_ok=True)

        torch.save(metadata_old, os.path.join('Output_subset_b2', f'render_far{z_vals_inbound}', 'metadata', f"{metadata_item.image_path.stem}.pt"))
