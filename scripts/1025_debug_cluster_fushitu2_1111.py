...
def _get_train_opts() -> Namespace:
    ...
    parser.add_argument('--all_thing_features_15', type=str, default='logs_campus/1107_campus_density_depth_hash22_instance_origin_sam_0.001_depth_crossview/1/eval_100000/panoptic/all_thing_features.npy',required=False, help='experiment name')
    parser.add_argument('--sem_rgb_15', type=str, default='logs_campus/1107_campus_density_depth_hash22_instance_origin_sam_0.001_depth_crossview/1/eval_100000/panoptic',required=False, help='experiment name')
    parser.add_argument('--all_thing_features_fushi', type=str, default='/data/yuqi/code/GP-NeRF-semantic/logs_campus/1107_campus_density_depth_hash22_instance_origin_sam_0.001_depth_crossview/2/eval_100000fushi/panoptic/all_thing_features.npy',required=False, help='experiment name')
    parser.add_argument('--sem_rgb_fushi', type=str, default='/data/yuqi/code/GP-NeRF-semantic/logs_campus/1107_campus_density_depth_hash22_instance_origin_sam_0.001_depth_crossview/2/eval_100000fushi/panoptic',required=False, help='experiment name')
...
def hello(hparams: Namespace) -> None:
    ...
    # writer = SummaryWriter(os.path.join('zyq_cluster_crossview_50W', 'tb'))
    writer = None
    num_images =11
    ...
    for cluster_size in [50,100,200]:
        ...
        output=f'1111_panoptic_{num_points}point_clustersize{cluster_size}_crossview_20W'

        Path(os.path.join('zyq_cluster_crossview_50W',output)).mkdir(exist_ok=True, parents=True)
        Path(os.path.join('zyq_cluster_crossview_50W',output,'pred_semantics')).mkdir(exist_ok=True)
        Path(os.path.join('zyq_cluster_crossview_50W',output,'pred_surrogateid')).mkdir(exist_ok=True)
        Path(os.path.join('zyq_cluster_crossview_50W',output,'gt_semantics')).mkdir(exist_ok=True)
        Path(os.path.join('zyq_cluster_crossview_50W',output,'gt_surrogateid')).mkdir(exist_ok=True)
        ...
        all_points_instances_15, centroids = cluster(all_thing_features, bandwidth=bandwidth, device=device, num_images=num_images, 
        ...
        all_points_semantics=torch.from_numpy(all_points_semantics).to(device).view(1, H*W)
        all_points_rgb=torch.from_numpy(all_points_rgb).to(device).view(1, H*W, -1)
        ...
        for save_i in range(1):
            ...
            # # 保存图像为文件（例如，PNG格式）
            # plt.savefig(os.path.join('zyq_cluster_crossview_50W', output,f'lda_visualization_{num_points}point_clustersize{cluster_size}.png'))
            ...
            Image.fromarray(output_semantics_with_invalid.reshape(H, W).cpu().numpy().astype(np.uint8)).save(
                os.path.join('zyq_cluster_crossview_50W', output , 'pred_semantics', ("%06d.png" % save_i)))
            
            Image.fromarray(p_instances.argmax(dim=1).reshape(H, W).cpu().numpy().astype(np.uint16)).save(
                os.path.join('zyq_cluster_crossview_50W', output , 'pred_surrogateid', ("%06d.png" % save_i)))
            ...
            Image.fromarray(grid).save(os.path.join('zyq_cluster_crossview_50W',output,("%06d_fushitu.jpg" % save_i)))

        output_dir = hparams.sem_rgb_15
        all_thing_features = np.load(os.path.join(output_dir, "all_thing_features.npy"))
        all_points_semantics = np.load(os.path.join(output_dir, "all_points_semantics.npy"))
        all_points_rgb = np.load(os.path.join(output_dir, "all_points_rgb.npy"))
        all_points_semantics=torch.from_numpy(all_points_semantics).to(device).view(num_images, H*W)
        all_points_rgb=torch.from_numpy(all_points_rgb).to(device).view(num_images, H*W, -1)
        ...
        gt_points_semantic=torch.from_numpy(gt_points_semantic).to(device).view(num_images, H*W)
        gt_points_rgb=torch.from_numpy(gt_points_rgb).to(device).view(num_images, H*W, -1)
        gt_points_instance=torch.from_numpy(gt_points_instance).to(device).view(num_images, H*W)

        save_i=0
        # for p_rgb, p_semantics, p_instances in zip(all_points_rgb, all_points_semantics, all_points_instances)
        for save_i in range(num_images):
            p_rgb = all_points_rgb[save_i]
            # p_semantics = all_points_semantics[save_i]
            ...
            Image.fromarray(gt_semantics.reshape(H, W).cpu().numpy().astype(np.uint8)).save(
                os.path.join('zyq_cluster_crossview_50W', output , 'gt_semantics', ("%06d.png" % save_i)))

            
            Image.fromarray(gt_instances.reshape(H, W).cpu().numpy().astype(np.uint16)).save(
                os.path.join('zyq_cluster_crossview_50W', output , 'gt_surrogateid', ("%06d.png" % save_i)))        
            ...
            Image.fromarray(grid).save(os.path.join('zyq_cluster_crossview_50W',output,("%06d.jpg" % save_i)))


        path_target_sem = os.path.join('zyq_cluster_crossview_50W',output,'gt_semantics')
        path_target_inst = os.path.join('zyq_cluster_crossview_50W',output,'gt_surrogateid')
        path_pred_sem = os.path.join('zyq_cluster_crossview_50W',output,'pred_semantics')
        path_pred_inst = os.path.join('zyq_cluster_crossview_50W',output,'pred_surrogateid')
        if Path(path_target_inst).exists():
            ...
            with(Path(os.path.join('zyq_cluster_crossview_50W',output, 'metric.txt'))).open('w') as f:
