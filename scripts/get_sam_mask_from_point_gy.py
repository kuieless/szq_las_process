...
def main() -> None:
    ...
    #feature = np.load('/data/guanying/Code/segment-anything/example_dataset/embeddings/000277.npy')[0]
    feature = np.load('/data/yuqi/Datasets/MegaNeRF/UrbanScene3D/residence/residence-labels/train/sam_features/000000.npy')
    ...
    while N_selected < 128:

        # random_point = torch.nonzero(bool_tensor)[torch.randint(high=torch.sum(bool_tensor), size=(1,))]
        random_point = [800, 400]
        #random_point = torch.tensor([[800, 400]]).to(device) # W H
        print(random_point)
        selected_one.append(random_point)
        # in_points = torch.tensor([[[int(W/2), int(H/2)]]]).to(device=device)
        sam_point = np.array([random_point[1], random_point[0]]).reshape(1, 2)
        masks, iou_preds, _ = predictor.predict(sam_point, in_labels[0], multimask_output=False)
        #masks, iou_preds, _ = predictor.predict_torch(sam_point, in_labels, multimask_output=False)
        print(iou_preds)
        import pdb; pdb.set_trace() 
        masks_max = masks.squeeze(0)
        ...
        if masks_max.nonzero().size(0) ==0:
            ...
            selected_points.append(select_point)
            N_selected += 128
        cv2.imwrite(f"mask_{N_selected}_gy.png", masks_max.cpu().numpy().astype(np.int32)*255)
    ...
    # 显示图像
    cv2.imwrite("mask_final_gy.png", (rgb_array_test).astype(np.int32))
