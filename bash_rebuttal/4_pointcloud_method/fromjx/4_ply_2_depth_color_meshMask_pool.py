...
def process_sample(args):
    split, metadata_list_dir, rgb_list_dir, meshMask_dir, points_homogeneous, down_scale, thres, \
        save_dir_imgCoverList, scalar_attribute, colors, alpha, save_dir_img, hparams = args
    log_name = metadata_list_dir[0].split('/')[-1][:-3]
    os.makedirs(f'{save_dir_img}/logging', exist_ok=True)
    logging.basicConfig(filename=f'{save_dir_img}/logging/process_from_{log_name}.log', level=logging.INFO)
    logging.info(f"Processing from: {log_name}")
    ...
    for metadata_file_dir, rgb_file_dir, meshMask_file_dir in tqdm(zip(metadata_list_dir, rgb_list_dir, meshMask_dir), \
        ...
        npdata = 0 * np.ones((image_height, image_width, 1), dtype=np.uint16)
        ...
        for threshold, save_dir_imgCoverThres in zip(thres, save_dir_imgCoverList):
            ...
            # image_flag = np.zeros((image_height, image_width), dtype=bool)
...
def _get_opts():
    ...
    parser.add_argument('--dataset_path', default='/data/yuqi/Datasets/DJI/Longhua_block1_20231020_ds',type=str, required=False)
    parser.add_argument('--output_path', default='/data/jxchen/dataset/dji/longhua/render_semantic_label_1113',type=str, required=False)
    parser.add_argument('--meshMask_path', default='/data/yuqi/Datasets/DJI/Longhua_block1_20231020_ds',type=str)
    # parser.add_argument('--txt_path', default='/data/jxchen/dji/Yingrenshi/dji_labeled_segmented_ply.txt',type=str)
    # 务必检查点云数据的第几列是语义或实例！可能和代码中的不统一
    parser.add_argument('--ply_path', default='/data/jxchen/dataset/dji/longhua/dji_labelOnly_segmented_ply_cloudEditted_2.txt', type=str)
    ...
    parser.add_argument('--instance', type=bool, default=False, help='')  
...
def main(hparams):
    ...
    if hparams.ply_path.split('.')[-1]=='txt':
        ...
        xyz = data[:, :3]
        if data.shape[1]==4:
            scalar_attribute = data[:, 3]
        elif hparams.instance:
            if txt_file[-16:-4] == 'cloudEditted':
                scalar_attribute = data[:, 6]  # CC编辑后instance和semantic顺序互换了, 但有时候可能semantic换到第八列去..玄学
            else:
                scalar_attribute = data[:, 7]
        else:
            if txt_file[-16:-4] == 'cloudEditted':
                scalar_attribute = data[:, 7]
            else:
                scalar_attribute = data[:, 6]  # xyz rgb semantic instance
        
        if hparams.instance:
            # 调整标量值，-100变为-1，然后所有标量+1
            scalar_attribute[scalar_attribute==-100.0] = -1.0
            scalar_attribute += 1
            ...
            scalar_mapping = {(x+1):random_colors[x] for x in range(100)}
            scalar_mapping[-1+1] = (0,0,0)  # 原本-100对应0，0，0  现在令其对应-1然后+1
        else:
            scalar_mapping = {
                0: (0, 0, 255),   # 蓝色 (Blue), terrain,2
                1: (0, 255, 0),   # 绿色 (Green), vegetation,4
                # 2: (128, 0, 128),  # 紫色 (Purple)
                3: (0, 0, 255),    # 蓝色 (Blue), terrain, 2； 3原本是bridge现在也对应2
                4: (255, 255, 0),  # 黄色 (Yellow), vehicle,3
                6: (255, 0, 0),    # 红色 (Red), building,1
                7: (0, 0, 255),   # 蓝色 (Blue), terrain,2
                8: (0, 255, 0),   # 绿色 (Green), vegetation,4
                # 2: (128, 0, 128),  # 紫色 (Purple)
                # 3: (255, 165, 0),  # 橙色 (Orange)
                11: (255, 255, 0),  # 黄色 (Yellow), vehicle,3
                13: (255, 0, 0),    # 红色 (Red), building,1
            }
        colors = np.zeros((len(scalar_attribute), 3), dtype=np.uint8)  # 初始化颜色数组
        for key, value in scalar_mapping.items():
            mask = (scalar_attribute == key)
            colors[mask] = value
        if not hparams.instance:
            scalar_attribute_old = scalar_attribute.copy()
            scalar_attribute[scalar_attribute_old==0] = 2
            scalar_attribute[scalar_attribute_old==7] = 2
            scalar_attribute[scalar_attribute_old==3] = 2
            scalar_attribute[scalar_attribute_old==1] = 4
            scalar_attribute[scalar_attribute_old==8] = 4
            scalar_attribute[scalar_attribute_old==4] = 3
            scalar_attribute[scalar_attribute_old==11] = 3
            scalar_attribute[scalar_attribute_old==6] = 1
            scalar_attribute[scalar_attribute_old==13] = 1
            print('lable changed according another rule!')

    elif hparams.ply_path.split('.')[-1]=='ply':
        ...
        if not (hparams.instance):
            ...
            # 生成50种随机颜色
            random_colors = [random_color() for _ in range(100)]
            color_mapping = {random_colors[x]:(x+1) for x in range(100)}
        ...
        scalar_attribute = scalar_attribute.squeeze()
    ...
    save_dir_img_meta = os.path.join(save_dir, "img")
    save_dir_imgEx = os.path.join(save_dir, "imgEx")
    save_dir_imgCoverThres60 = os.path.join(save_dir, "alphaCover_thres60")
    save_dir_imgCoverThres50 = os.path.join(save_dir, "alphaCover_thres50")
    save_dir_imgCoverThres40 = os.path.join(save_dir, "alphaCover_thres40")
    ...
    save_dir_imgCoverThres05 = os.path.join(save_dir, "alphaCover_thres05")
    save_dir_imgCoverThres02 = os.path.join(save_dir, "alphaCover_thres02")
    save_dir_imgCoverThres01 = os.path.join(save_dir, "alphaCover_thres01")
    # save_dir_imgCoverList = [save_dir_imgCoverThres30, save_dir_imgCoverThres20, save_dir_imgCoverThres15, 
    #                         save_dir_imgCoverThres10, save_dir_imgCoverThres05, save_dir_imgCoverThres02, 
    #                         save_dir_imgCoverThres01]
    # thres = [0.03, 0.02, 0.015, 0.01, 0.005, 0.002, 0.001]
    # save_dir_imgCoverList = [save_dir_imgCoverThres60,save_dir_imgCoverThres50,save_dir_imgCoverThres40]
    # thres = [0.06, 0.05, 0.04]
    save_dir_imgCoverList = [save_dir_imgCoverThres20]
    thres = [0.02]

    os.makedirs(save_dir, exist_ok=True)
    os.makedirs(save_dir_img_meta, exist_ok=True)
    os.makedirs(save_dir_imgEx, exist_ok=True)
    os.makedirs(save_dir_imgCoverThres60, exist_ok=True)
    os.makedirs(save_dir_imgCoverThres50, exist_ok=True)
    os.makedirs(save_dir_imgCoverThres40, exist_ok=True)
    ...
    os.makedirs(save_dir_imgCoverThres05, exist_ok=True)
    os.makedirs(save_dir_imgCoverThres02, exist_ok=True)
    os.makedirs(save_dir_imgCoverThres01, exist_ok=True)
    ...
    for split in ("val", "train"):
        ...
        save_dir_img = os.path.join(save_dir_img_meta, split)

        # Divide dataset into num_parts equal parts
        num_parts = 16
        subset_size = len(metadata_list_dir) // num_parts
        if subset_size == 0:
            num_parts = len(metadata_list_dir) - 1
            subset_size = len(metadata_list_dir) // num_parts
        # Create a list of arguments for parallel processing
        args_list = []
        for i in range(num_parts):
            start_idx = i * subset_size
            end_idx = (i + 1) * subset_size if i < num_parts - 1 else len(metadata_list_dir)
            subset_metadata_list_dir = metadata_list_dir[start_idx:end_idx]
            subset_rgb_list_dir = rgb_list_dir[start_idx:end_idx]
            subset_meshMask_dir = meshMask_dir[start_idx:end_idx]
            args_list.append((split, subset_metadata_list_dir, subset_rgb_list_dir, subset_meshMask_dir, points_homogeneous, \
                down_scale, thres, save_dir_imgCoverList, scalar_attribute, colors, alpha, save_dir_img, hparams))

        # Create a pool of worker processes and process the data in parallel
        # process_sample(args_list[0])
        num_processes = 16 if num_parts>=16 else num_parts
