...
def ypr_to_opk(input):
    ...
    # r_camera = Rotation.from_euler('XYZ',[0,0,180],degrees=True)
...
def main(hparams):
    ...
    for i in range(len(c2w_R)):
        ...
        c2w.append(temp)
    c2w = np.array(c2w)
    visualize_poses_and_points(c2w,mesh=trimesh.load_mesh('/data/yuqi/Datasets/InstanceBuilding/3D/scene1/scene1.obj'))
