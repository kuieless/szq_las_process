...
def hello(hparams: Namespace) -> None:
    ...
    if 'seq' in hparams.dataset_path:
    ...
    if not os.path.exists(os.path.join(output_path, 'replace_building_label')):
        Path(os.path.join(output_path, 'replace_building_label')).mkdir(parents=True)
    ...
    for metadata_item in tqdm(train_items, desc="replace building"):
        ...
        # 取出 only_sam_m2f_far_project_path 中的building，覆盖掉 ori_m2f 的对应区域(除了原图中的car)
        building_mask = (far_m2f==1) * (ori_m2f != 3)

        replace_building_label = ori_m2f.clone()
        replace_building_label[building_mask] = 1

        Image.fromarray(replace_building_label.numpy().astype(np.uint16)).save(os.path.join(output_path, 'replace_building_label', f"{file_name}.png"))
        

        color_label = custom2rgb(replace_building_label.numpy())
        ...
        cat = torch.hstack([ori_m2f, replace_building_label])
