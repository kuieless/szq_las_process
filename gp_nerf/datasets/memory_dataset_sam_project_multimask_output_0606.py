...
class MemoryDataset_SAM(Dataset):
    ...
    def __getitem__(self, idx) -> Dict[str, torch.Tensor]:
        ...
        if self.select_origin == True:
            ...
            # random_point[0, 1] = int(self.W * 4/5)
            # random_point[0, 0] = int(self.H * 2/3)
        ...
        #从初始点采样 sam mask
        a = random_point.cpu().numpy()
        masks, iou_preds, masks_low = self.predictor.predict(a, in_labels, return_torch=True)
        masks_max = masks[:, torch.argmax(iou_preds),:,:].squeeze(0)
        masks_low_max = masks_low[:, torch.argmax(iou_preds),:,:]
        masks_max = masks_max * ~bool_tensor.to(self.device)
        Image.fromarray(masks_max.cpu().numpy()).save("mask1.jpg")



        # img_cv=  np.asarray((masks_max.int()*255).cpu().numpy().astype(np.uint8), order="C")
        # # thresh = cv2.threshold(img_cv, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
        # ret, labels_c = cv2.connectedComponents(img_cv, connectivity=4)
        # unique_c, counts_c = np.unique(labels_c[labels_c.nonzero()], return_counts=True)
        # labels_max_idx = np.argmax(counts_c)
        
        # masks_max_label = torch.zeros((self.H, self.W), dtype=torch.int).bool()
        # masks_max_label[labels_c==unique_c[labels_max_idx]] = True

        # sam_points = torch.nonzero(masks_max_label)[torch.randint(high=torch.sum(masks_max_label), size=(200,))]
        # sam_points = sam_points.flip(1).cpu().numpy()
        # masks, iou_preds, _ = self.predictor.predict(sam_points, np.array([1]*sam_points.shape[0]), return_torch=True)
        # masks_max1 = masks[:, torch.argmax(iou_preds),:,:].squeeze(0)
        # masks_max1 = masks_max1 * ~bool_tensor.to(self.device)     
        # visual = masks_max1
        # visual = visual.cpu().numpy()
        # rgb_array = np.stack([visual, visual, visual], axis=2)* 255
        # sam_points1 = torch.from_numpy(sam_points).flip(1)
        # for k in range(sam_points1.shape[0]):  # H, W
        #     x, y = sam_points1[k, 0], sam_points1[k, 1]
        #     for m in range(-3,3):
        #         for n in range(-3,3):
        #             if x+m < H and x+m >0 and y+n < W and y+n >0:
        #                 rgb_array[x+m, y+n] = np.array([0, 0, 128])
        # cv2.imwrite("mask4.jpg", rgb_array)
        # print("save mask4")



        sam_points = torch.nonzero(masks_max)[torch.randint(high=torch.sum(masks_max), size=(10,))]
        sam_points = sam_points.flip(1).cpu().numpy()


        masks, iou_preds, _ = self.predictor.predict(sam_points, np.array([1]*sam_points.shape[0]), multimask_output=False, return_torch=True)
        masks_max1 = masks[:, torch.argmax(iou_preds),:,:].squeeze(0)
        masks_max1 = masks_max1 * ~bool_tensor.to(self.device)     
        ...
        for k in range(sam_points1.shape[0]):  # H, W
            ...
            for m in range(-3,3):
                for n in range(-3,3):
                    if x+m < H and x+m >0 and y+n < W and y+n >0:
                        rgb_array[x+m, y+n] = np.array([0, 0, 128])
        cv2.imwrite("mask2.jpg", rgb_array)
        # Image.fromarray(rgb_array).save("mask3.jpg")
        print("save mask2")


        iiii=1
        img_list=[]
        masks_low_max1= masks_low_max
        for sam_points_multi in sam_points:
            # sam_points_multi = sam_points_multi[np.newaxis,:]
            # masks, iou_preds, masks_low = self.predictor.predict(sam_points_multi, in_labels, mask_input=masks_low_max.cpu().numpy(), return_torch=True)

            sam_points_multi = sam_points[:iiii]
            masks, iou_preds, masks_low = self.predictor.predict(sam_points_multi, np.array([1]*sam_points_multi.shape[0]), multimask_output=False, mask_input=masks_low_max1.cpu().numpy(), return_torch=True)
            masks_max1 = masks[:, torch.argmax(iou_preds),:,:].squeeze(0)
            masks_low_max1 = masks_low[:, torch.argmax(iou_preds),:,:]
            masks_max1 = masks_max1 * ~bool_tensor.to(self.device)     
            ...
            visual = visual.cpu().numpy()
            rgb_array = np.stack([visual, visual, visual], axis=2)* 255
            sam_points1 = torch.from_numpy(sam_points_multi).flip(1)
            for k in range(sam_points1.shape[0]):  # H, W
                x, y = sam_points1[k, 0], sam_points1[k, 1]
                for m in range(-3,3):
                    for n in range(-3,3):
                        if x+m < H and x+m >0 and y+n < W and y+n >0:
                            rgb_array[x+m, y+n] = np.array([128, 0, 128])
            img_list.append(torch.from_numpy(rgb_array))

            iiii = iiii + 1
            # cv2.imwrite(f"mask3_{iiii}.jpg", rgb_array)
            # print("save mask3")
        from torchvision.utils import make_grid
        img_list = torch.stack(img_list).permute(0,3,1,2)
        img_list = make_grid(img_list, nrow=3, padding=1)
        img_grid = img_list.permute(1, 2, 0).cpu().numpy().astype(np.uint8)
        Image.fromarray(img_grid).save(f"mask3_{iiii}.jpg")
        print(f"mask3_{iiii}.jpg")
        

        # b = np.array([[540,600]])
        # c = np.concatenate([a,b])
        # masks, iou_preds, _ = self.predictor.predict(c, np.array([1,1]), return_torch=True)

        # masks_max = masks[:, torch.argmax(iou_preds),:,:].squeeze(0)
        # masks_max = masks_max * ~bool_tensor.to(self.device)                                         
        # Image.fromarray(masks_max.cpu().numpy()).save("mask2.jpg")
