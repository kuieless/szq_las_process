...
def get_rgb_index_mask_depth_dji_instance(metadata: ImageMetadata) -> Optional[
    ...
    return rgbs, metadata.image_index, keep_mask, instance, depth_dji
...
def get_rgb_index_mask_monocues(metadata: ImageMetadata) -> Optional[
    ...
    if metadata.normal_path is not None:
        normals = metadata.load_normal().view(-1, 3)
        if keep_mask is not None:
            normals = normals[keep_mask == True]
    ...
    assert metadata.image_index <= torch.iinfo(torch.int32).max
    return rgbs, metadata.image_index, keep_mask, labels, normals, depths
