    ...
    def __getitem__(self, idx) -> Dict[str, torch.Tensor]:
        ...
        if self.select_origin == True:
            ...
            for point_idx in range(len(self.world_points)):
                ...
                bool_tensor = self._labels[idx].bool()
                pt_3d_trans = np.dot(w2c, self.world_points[point_idx])
                ...
                if x2 >= 0 and x2 < self.W and y2 >= 0 and y2 < self.H:
                    ...
                    if self.visualization:
                        ...
                        if depth_diff > occluded_threshold:
                            ...
                            # cv2.imwrite(f"{save_dir}/{self.object_ids[point_idx]}_{self.metadata_items[idx].image_path.stem}_project_{depth_diff}.jpg", img)
