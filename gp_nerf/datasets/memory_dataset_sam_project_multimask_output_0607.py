    ...
    def __getitem__(self, idx) -> Dict[str, torch.Tensor]:
        ...
        if self.select_origin == True:
            ...
            for random_point in self.random_points:
                ...
                # if self.visualization:
                #         pt2 = (int(random_point[0, 0]), int(random_point[0, 1]))
                #         radius = 5
                #         color = (0, 0, 255)
                #         thickness = 2
            ...
            for point_idx in range(len(self.world_points)):
                ...
                self._labels[idx][masks_max] = self.object_ids[point_idx]
                ...
                #     for k in range(selected_point.size(0)):  # H, W
                #         x, y = selected_point[k, 0], selected_point[k, 1]
                #         if x < H and x >0 and y < W and y >0:
                #             rgb_array[x, y] = np.array([0, 128, 0])

                #     image_cat = np.concatenate([img, visual_pseudo, rgb_array], axis=1)
                #     cv2.imwrite(f"{save_dir}/{self.object_ids[point_idx]}_{self.metadata_items[idx].image_path.stem}_project_sample.jpg", image_cat)
                #     print(f"{save_dir}/{self.object_ids[point_idx]}_{self.metadata_items[idx].image_path.stem}_project_sample.jpg")
                # # *****************************************************************************
            if self.visualization and len(self.sam_points) > 0:
                rgb_array = np.stack([self._labels[idx].bool(), self._labels[idx].bool(), self._labels[idx].bool()], axis=2)* 255
                image_cat = np.concatenate([img, rgb_array], axis=1)
                cv2.imwrite(f"{save_dir}/{self.object_ids[point_idx]}_{self.metadata_items[idx].image_path.stem}_project_{depth_diff}.jpg", image_cat)
                print(f"{save_dir}/{self.object_ids[point_idx]}_{self.metadata_items[idx].image_path.stem}_project_{depth_diff}.jpg")
        ...
        #从整张图像中采样
        bool_tensor = self._labels[idx].bool().to(self.device)
        ...
        if self.visualization and len(self.sam_points) > 0:
            ...
            image_cat = np.concatenate([img, visual_pseudo, rgb_array], axis=1)
            cv2.imwrite(f"{save_dir}/{self.metadata_items[idx].image_path.stem}_project_sample.jpg", image_cat)
            print(f"{save_dir}/{self.metadata_items[idx].image_path.stem}_project_sample.jpg")
