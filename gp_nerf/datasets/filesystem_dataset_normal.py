"""
Suport only loading normal, no depth
"""
import math
import os
import shutil
...
class FilesystemDataset(Dataset):
    ...
    def load_chunk(self) -> None:
        chosen, self._loaded_rgbs, self._loaded_rays, self._loaded_img_indices, self._labels, self._normals = self._chunk_future.result()
        self._chosen = chosen
        self._chunk_future = self._chunk_load_executor.submit(self._load_chunk_inner)
    ...
    def __getitem__(self, idx) -> Dict[str, torch.Tensor]:
        ...
        if self._normals is not None:
            data['normals'] = self._normals[idx]
        if self._depths is not None:
            data['depth'] = self._depth[idx]
        return data
    ...
    def _load_chunk_inner(self) -> Tuple[str, torch.FloatTensor, torch.FloatTensor, torch.ShortTensor]:
        ...
        normals = None
        #if metadata_items[0].normal_path is not None:
        if 'normals_0' in loaded_chunk.to_pandas():
            normals = torch.FloatTensor(loaded_chunk.to_pandas()[['normals_{}'.format(i) for i in range(3)]].to_numpy())
        
        return str(chosen), rgbs, loaded_rays, loaded_img_indices, loaded_labels, normals
    ...
    def _write_chunks(self, metadata_items: List[ImageMetadata], center_pixels: bool, device: torch.device,
        ...
        for chunk_path, path_free in zip(chunk_paths, path_frees):
            ...
            for j in range(allocated):
                ...
                dtypes.append(('labels', pa.from_numpy_dtype(img_indices_dtype)))
        ...
        with ThreadPoolExecutor(max_workers=len(parquet_writers)) as executor:
            for metadata_item in main_tqdm(metadata_items):
                ...
                if metadata_item.normal_path is not None:
                    normals.append(normal.clone())
                ...
                if in_memory_count >= disk_flush_size:
                    ...
                    normals_cat = torch.cat(normals) if len(normals) > 0 else None
                    write_futures = self._write_to_disk(executor, torch.cat(rgbs), torch.cat(rays), torch.cat(indices),
                                                        parquet_writers, img_indices_dtype, torch.cat(labels), normals_cat, desired_chunks)
    ...
    def _write_to_disk(self, executor: ThreadPoolExecutor, rgbs: torch.Tensor, rays: torch.FloatTensor,
                       ...
                       img_indices_dtype: Type[Union[np.ushort, np.int]], labels: torch.Tensor, normals: torch.HalfTensor, desired_chunks=1000) -> List[Future[None]]:
        ...
        def append(index: int) -> None:
            ...
            columns["labels"] = shuffled_labels[index * chunk_size:(index + 1) * chunk_size].numpy().astype(
                    img_indices_dtype)
            ...
            if normals is not None:
                for i in range(normals.shape[1]):
                    columns['normals_{}'.format(i)] = shuffled_normals[index * chunk_size:(index + 1) * chunk_size, i].numpy()
