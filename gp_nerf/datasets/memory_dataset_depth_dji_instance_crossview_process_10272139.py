...
class MemoryDataset(Dataset):
    ...
    def __getitem__(self, idx) -> Dict[str, torch.Tensor]:
        ...
        for idx_next in index_list:
            ...
            unique_label = torch.unique(instances_current)
            for uni in unique_label:
                mask = instances_current == uni
                mask_area = mask.sum()
                
                label_in_mask = project_instance[mask]
                ...
                for uni_2 in uni_label_in_mask:
                    ...
                    mask_area_overlap = (mask * mask_2).sum
                    if (mask_area_overlap / mask_area_2) > overlap_threshold or (mask_area_overlap / mask_area):
