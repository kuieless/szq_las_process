...
def hello(data_dir, n_out_poses, key_poses):
    ...
    if not os.path.exists(os.path.join('/data/yuqi/Datasets/DJI/Longhua_block2_20231020_ds', 'render_supp', 'metadata')):
        Path(os.path.join('/data/yuqi/Datasets/DJI/Longhua_block2_20231020_ds', 'render_supp', 'metadata')).mkdir(parents=True, exist_ok=True)

    for i in range(len(out_poses)):
        metadata_old['c2w'] = torch.FloatTensor(out_poses[i])
        torch.save(metadata_old, os.path.join('/data/yuqi/Datasets/DJI/Longhua_block2_20231020_ds', 'render_supp', 'metadata', f"{i:06d}.pt"))
