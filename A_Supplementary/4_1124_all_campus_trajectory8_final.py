...
@click.option('--key_poses', type=str, default='631,236,592,599,150,141,811')
...
def hello(data_dir, n_out_poses, key_poses):
    ...
    ###  1. 设定第一帧的位置
    key1= key_poses_all[2]

    ## 选了part2的中间插值部分的相机，正对场景的长轴，打算把位置替换成far视角
    inter1_out = inter_poses(key_poses_all[0:2], n_out_poses)
    inter1=inter1_out[40]

    ## 接下来找far视角
    metadata_item=train_items[key_poses[2]]
    ...
    traj2 = np.concatenate([key3[np.newaxis,:],key2[np.newaxis,:]],0)
    out_poses2 = inter_poses(traj2, 15)

    ##  6.mp4 后在far的高度平飞
    key4_rotation = inter1_out[68][:3,:3]
    metadata_item=train_items[key_poses[3]]

    pose4= metadata_item.c2w.numpy()
    key4_position = pose4[:3,3]
    key4_position[0] = pose4[0,3] # + 0.6 * (key2[0,3]-pose4[0,3])
    ...
    out_poses3 = inter_poses(traj3, 48)
    ...
    out_poses4 = inter_poses(traj4, 72)
    ...
    key6=key_poses_all[5]
    key6[:3,:3]=rotation_matrix_y @ (rotation_matrix_x @ key6[:3,:3])
    key6[0,3] = 2 * key6[0,3]

    traj5 = np.concatenate([out_poses4[-1:],key6[np.newaxis,:]],0)
    out_poses5 = inter_poses(traj5, 36)
    ...
    rotation_matrix_y = torch.tensor([[cosine, 0, sine],
                    [0, 1, 0],
                    [-sine, 0, cosine]])
    key7_rotation = inter1_out[68][:3,:3]
    key7_rotation=rotation_matrix_y @ (rotation_matrix_x @ key7_rotation)

    key7= key_poses_all[6]
    key7[:3,:3] = key7_rotation
