...
@click.option('--key_poses', type=str, default='96,119,230,446')
...
def hello(data_dir, n_out_poses, key_poses):
    ...
    rotation_matrix_x = torch.tensor([[1, 0, 0],
                    [0, cosine, sine],
                    [0, -sine, cosine]])
    rotation_3 = rotation_matrix_y @ (rotation_matrix_x @ key2[:3,:3])

    key6=key_poses_all[3]
    key6[:3,:3]=rotation_3
    traj5 = np.concatenate([key5[np.newaxis,:],key6[np.newaxis,:]],0)
    out_poses5 = inter_poses(traj5, 15)
    out_poses5 = out_poses5[:int(len(out_poses5)*0.4)]



    # out_poses = np.concatenate([out_poses1, out_poses2, out_poses3, out_poses4,out_poses5],0)
    out_poses = np.concatenate([out_poses4,out_poses5],0)
